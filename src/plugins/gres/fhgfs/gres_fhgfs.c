/*****************************************************************************\
 *  gres_fhgfs.c - Support FhGFS-OnDemand as a generic resources.
 *****************************************************************************
 *  Copyright (C) 2014 HPCNow!
 *  Written by Jordi Blasco-Pallares
 ...
 */
const char	plugin_name[]		= "Gres FhGFS plugin";
const char	plugin_type[]		= "gres/fhgfs";
const uint32_t	plugin_version		= 1;

static char	gres_name[]		= "fhgfs";

static int *fhgfs_instances = NULL;
static int nb_available_instances;
...
extern int node_config_load(List gres_conf_list)
{
	...
	int nb_fhgfs = 0;	/* Number of FhGFS server instances available per node (DEFAULT 1) */
	...
	fhgfs_instances = NULL;
	nb_available_instances = -1;

	/* (Re-)Allocate memory if number of files changed */
	if (nb_fhgfs != nb_available_instances) {
		xfree(fhgfs_instances);	/* No-op if NULL */
		fhgfs_instances = (int *) xmalloc(sizeof(int) * nb_fhgfs);
		nb_available_instances = nb_fhgfs;
		for (i = 0; i < nb_available_instances; i++)
			fhgfs_instances[i] = -1;
	}
	...
	for (i = 0; i < nb_available_instances; i++)
		info("FhGFS instance %d is instance number %d", i, fhgfs_instances[i]);
	...
	if ((gres_job_ptr != NULL) &&
		...
		for (i=0; i<len; i++) {
			if (!bit_test(gres_job_ptr->gres_bit_alloc[0], i))
				continue;
			if (!instance_list)
				instance_list = xmalloc(128);
			else
				xstrcat(instance_list, ",");
			if (fhgfs_instances && (fhgfs_instances[i] >= 0))
				xstrfmtcat(instance_list, "%d", fhgfs_instances[i]);
			else
				xstrfmtcat(instance_list, "%d", i);
		}
	}
	if (instance_list) {
		env_array_overwrite(job_env_ptr,"FhGFS",
				    instance_list);
		xfree(instance_list);
	} else {
		error("gres/fhgfs unable to set FhGFS server instances, "
		      "no FhGFS filesystem was setup for this job");
	}
}
...
extern void send_stepd(int fd)
{
	...
	safe_write(fd, &nb_available_instances, sizeof(int));
	for (i = 0; i < nb_available_instances; i++)
		safe_write(fd, &fhgfs_instances[i], sizeof(int));
	return;
...
extern void recv_stepd(int fd)
{
	...
	safe_read(fd, &nb_available_instances, sizeof(int));
	if (nb_available_instances > 0)
		fhgfs_instances = xmalloc(sizeof(int) * nb_available_instances);
	for (i = 0; i < nb_available_instances; i++)
		safe_read(fd, &fhgfs_instances[i], sizeof(int));
	return;
